...
const DOCUMENT_PATTERNS = {
  T4: {
    ...
    extractors: {
      employer: /(?:Employer|Payer)[:\s]+([A-Z][A-Za-z\s&.,'-]+(?:Inc|Ltd|Corp|Co)?\.?)/i,
  ...
  RL1: {
    ...
    extractors: {
      employer: /(?:Employer|Payer)[:\s]+([A-Z][A-Za-z\s&.,'-]+(?:Inc|Ltd|Corp|Co)?\.?)/i,
      income: /(?:Case A|Box A)[:\s]+.*?\$?\s*([\d,]+\.?\d*)/i,
      qpp: /(?:Case B\.A|Box B\.A|QPP|RRQ)[:\s]*\$?\s*([\d,]+\.?\d*)/i,
      ei: /(?:Case C|Box C|EI|A-E)[:\s]*\$?\s*([\d,]+\.?\d*)/i,
      ppip: /(?:Case H|Box H|PPIP|RQAP)[:\s]*\$?\s*([\d,]+\.?\d*)/i,
      taxDeducted: /(?:Case E|Box E|Quebec income tax|Impôt du Québec)[:\s]*\$?\s*([\d,]+\.?\d*)/i,
    },
  },
  ...
  T4A: {
    ...
    extractors: {
      payer: /(?:Payer)[:\s]+([A-Z][A-Za-z\s&.,'-]+(?:Inc|Ltd|Corp|Co)?\.?)/i,
