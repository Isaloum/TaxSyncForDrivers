...
- **Project Name:** Tax Document Processing System
- **AWS Region:** us-east-1
- **Architecture:** Event-driven, serverless document processing pipeline
- **Primary Use Case:** Processing email attachments (especially tax documents) using AWS Textract
...
- **Purpose:** Grants Lambda functions access to AWS services for document processing
- **Trust Policy:** AWS Lambda service (lambda.amazonaws.com)
- **Session Duration:** 1 hour max

**Attached Policies:**
- **LambdaTextractS3Policy (Managed):**
...
- **ProcessEmailAttachmentPolicy (Inline):**
...
1. **Email Reception:** Amazon SES receives emails and stores them in the S3 bucket.
2. **Event Trigger:** S3 object creation triggers the Lambda function.
3. **Document Processing:** Lambda function processes email attachments using Textract.
4. **Data Storage:** Processing results are stored in the DynamoDB table.
5. **Notification:** SNS sends notifications about processing status.
6. **Logging:** All activities are logged to CloudWatch.

---

## Key Features

- Automated email processing (SES integration)
- Document text extraction (AWS Textract)
- Metadata tracking (DynamoDB)
- Cost optimization (S3 lifecycle policies)
- Security (IAM roles, blocked public access)
...
This documentation provides a complete overview of your serverless AWS document processing system, including architecture, security, workflow, and cost optimization measures.
